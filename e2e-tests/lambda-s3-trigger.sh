...
COMMON_ATTRIBUTES=$(<common_attribute.json)
  ...
  deploy_s3_trigger_stack "$LAMBDA_TEMPLATE_BUILD_DIR/$LAMBDA_TEMPLATE" "$S3_PREFIX" "$NEW_RELIC_LICENSE_KEY" "$NEW_RELIC_REGION" "$NEW_RELIC_ACCOUNT_ID" "false" "$S3_BUCKET_NAMES" "$COMMON_ATTRIBUTES"
  validate_stack_deployment_status "$S3_PREFIX"
  validate_lambda_s3_trigger_created "$S3_PREFIX" "$S3_BUCKET_NAME" "$S3_BUCKET_PREFIX"
...
COMMON_ATTRIBUTES=$(<common_attribute.json)
  ...
  deploy_s3_trigger_stack "$LAMBDA_TEMPLATE_BUILD_DIR/$LAMBDA_TEMPLATE" "$S3_SECRET_MANAGER" "$NEW_RELIC_LICENSE_KEY" "$NEW_RELIC_REGION" "$NEW_RELIC_ACCOUNT_ID" "true" "$S3_BUCKET_NAMES" "$COMMON_ATTRIBUTES"
  validate_stack_deployment_status "$S3_SECRET_MANAGER"
  validate_lambda_s3_trigger_created "$S3_SECRET_MANAGER" "$S3_BUCKET_NAME_SECRET_MANAGER" "$S3_BUCKET_PREFIX"
  upload_file_to_s3_bucket "$S3_BUCKET_NAME_SECRET_MANAGER" "$S3_BUCKET_OBJECT_NAME" "$S3_BUCKET_PREFIX" "$log_message"
  validate_logs_in_new_relic "$NEW_RELIC_USER_KEY" "$NEW_RELIC_ACCOUNT_ID" "$ATTRIBUTE_KEY_S3" "$S3_BUCKET_PREFIX" "$log_message"
  delete_stack "$S3_SECRET_MANAGER"
}
...
EOF
  ...
  validate_lambda_s3_trigger_not_created "$S3_SECRET_INVALID_BUCKET" "$S3_BUCKET_PREFIX_INVALID" "$S3_BUCKET_PREFIX"
  delete_stack "$S3_SECRET_INVALID_BUCKET"
}
