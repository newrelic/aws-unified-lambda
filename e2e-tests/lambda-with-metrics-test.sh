...
EOF
  ...
  deploy_firehose_metric_polling_stack "$S3_BUCKET_NAMES" "$LOG_GROUP_NAMES" "$COMMON_ATTRIBUTES" "false"
...
EOF
  ...
  deploy_firehose_metric_streaming_stack "$S3_BUCKET_NAMES" "$LOG_GROUP_NAMES" "$COMMON_ATTRIBUTES" "false"
...
EOF
  ...
  deploy_lambda_firehose_metric_polling_stack "$S3_BUCKET_NAMES" "$LOG_GROUP_NAMES" "$COMMON_ATTRIBUTES" "false"
...
EOF
  ...
  deploy_lambda_firehose_metric_streaming_stack "$S3_BUCKET_NAMES" "$LOG_GROUP_NAMES" "$COMMON_ATTRIBUTES" "false"
  validate_stack_deployment_status "$LAMBDA_FIREHOSE_METRIC_STREAMING_CASE"
  delete_stack "$LAMBDA_FIREHOSE_METRIC_STREAMING_CASE"
}

test_for_lambda_metric_polling_stack() {
cat <<EOF > cloudwatch-parameter.json
'[{"LogGroupName":"$LOG_GROUP_NAME","FilterPattern":"$LOG_GROUP_FILTER_PATTERN"}]'
EOF
...
EOF
  ...
  deploy_lambda_metric_polling_stack "$S3_BUCKET_NAMES" "$LOG_GROUP_NAMES" "$COMMON_ATTRIBUTES" "false"
  validate_stack_deployment_status "$LAMBDA_METRIC_POLLING_CASE"
  delete_stack "$LAMBDA_METRIC_POLLING_CASE"
}

test_for_lambda_metric_streaming_stack() {
cat <<EOF > cloudwatch-parameter.json
'[{"LogGroupName":"$LOG_GROUP_NAME","FilterPattern":"$LOG_GROUP_FILTER_PATTERN"}]'
EOF
...
EOF
  ...
  deploy_lambda_metric_streaming_stack "$S3_BUCKET_NAMES" "$LOG_GROUP_NAMES" "$COMMON_ATTRIBUTES" "false"
