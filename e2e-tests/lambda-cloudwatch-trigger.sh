...
COMMON_ATTRIBUTES=$(<common_attribute.json)
  ...
  deploy_cloudwatch_trigger_stack "$LAMBDA_TEMPLATE_BUILD_DIR/$LAMBDA_TEMPLATE" "$CLOUDWATCH_FILTER_PATTERN" "$NEW_RELIC_LICENSE_KEY" "$NEW_RELIC_REGION" "$NEW_RELIC_ACCOUNT_ID" "false"  "$LOG_GROUP_NAMES" "$COMMON_ATTRIBUTES"
  validate_stack_deployment_status "$CLOUDWATCH_FILTER_PATTERN"
  validate_lambda_subscription_created "$CLOUDWATCH_FILTER_PATTERN" "$LOG_GROUP_NAME" "$LOG_GROUP_FILTER_PATTERN"
  ...
  delete_stack "$CLOUDWATCH_FILTER_PATTERN"
}
...
COMMON_ATTRIBUTES=$(<common_attribute.json)
  ...
  deploy_cloudwatch_trigger_stack "$LAMBDA_TEMPLATE_BUILD_DIR/$LAMBDA_TEMPLATE" "$CLOUDWATCH_SECRET_MANAGER" "$NEW_RELIC_LICENSE_KEY" "$NEW_RELIC_REGION" "$NEW_RELIC_ACCOUNT_ID" "true"  "$LOG_GROUP_NAMES" "$COMMON_ATTRIBUTES"
  validate_stack_deployment_status "$CLOUDWATCH_SECRET_MANAGER"
  validate_lambda_subscription_created "$CLOUDWATCH_SECRET_MANAGER" "$LOG_GROUP_NAME_SECRET_MANAGER" "$LOG_GROUP_FILTER_PATTERN"
  ...
  delete_stack "$CLOUDWATCH_SECRET_MANAGER"
}
...
EOF
  ...
  validate_lambda_subscription_not_created "$CLOUDWATCH_INVALID_LOG_GROUP" "$LOG_GROUP_NAME_INVALID" "$LOG_GROUP_FILTER_PATTERN"

  delete_stack "$CLOUDWATCH_INVALID_LOG_GROUP"
}
